# 15. NumPy Foundations

> **Goal:** ndarrays, shapes, dtypes, vectorization, broadcasting, indexing, aggregation, linear algebra, and performance.
...
NumPy arrays store homogeneous data efficiently and expose operations implemented in optimized native code. Instead of:

```python
result = [x * 2 for x in values]
```

for numerical arrays you can often use:

```python
result = values * 2
```

Broadcasting allows compatible shapes to interact without manually writing nested loops. Always inspect `shape` and `dtype` when debugging numerical code.

# 01. Python Mental Model

> **Goal:** How Python executes code, objects, names, expressions, statements, REPL, scripts, modules, and how to think like a programmer.
...
## Deep dive: names, objects, and references

Python variables are better understood as **names bound to objects** than as boxes containing values. This matters when two names refer to the same mutable object.
...
`a` and `b` refer to the same list. Compare this with creating a copy: `b = a.copy()`. Understanding references prevents subtle bugs in data preprocessing and configuration objects.

## Execution model

At a high level, Python parses source code, compiles it to bytecode, and executes that bytecode in the Python runtime. You normally do not need to manipulate bytecode, but knowing that execution has runtime costs helps explain why pure Python loops can be slower than vectorized NumPy operations.

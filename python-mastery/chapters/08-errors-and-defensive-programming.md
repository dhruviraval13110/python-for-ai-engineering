# 08. Errors and Defensive Programming

> **Goal:** Exceptions, custom exceptions, validation, assertions, logging, retries, and designing failure-safe programs.
...
Catch exceptions at the level where you can actually handle them. A parser can raise a useful `ValueError`; an application boundary can convert that failure into a user-facing message or logged event. Avoid:
...
This silently converts a visible failure into corrupted state.

### Custom exceptions

```python
class InvalidDatasetError(ValueError):
    """Raised when a dataset violates required schema rules."""
...
def validate_columns(columns: set[str]) -> None:
    ...
    if missing:
        raise InvalidDatasetError(f"Missing columns: {sorted(missing)}")
```

This becomes valuable when multiple pipeline layers need to distinguish invalid input from programming errors.

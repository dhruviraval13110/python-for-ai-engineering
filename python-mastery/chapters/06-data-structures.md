...
> **Goal:** Lists, tuples, sets, dictionaries, slicing, unpacking, comprehensions, nested data, and choosing structures.
...
Use a **list** for ordered collections that may change. Use a **tuple** for fixed structured values. Use a **set** when uniqueness and membership are central. Use a **dict** when values are naturally accessed by keys.

Typical average-case membership behavior is different: list membership is linear, while hash-table-backed set/dict membership is typically constant-time on average. This is why replacing repeated `x in list` checks with a set can materially improve a data pipeline.
...
Do not choose a structure only because it is familiar. Choose it based on access patterns, mutability, ordering requirements, and readability.

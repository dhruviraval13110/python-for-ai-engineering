...
> **Goal:** Parameters, return values, scope, defaults, keyword-only arguments, annotations, closures, recursion, and API design.
...
## Deep dive: function design

A good function has a narrow responsibility and a predictable contract. Prefer returning values instead of changing unrelated global state. Use keyword-only parameters when they make calls safer:
...
The `*` means callers must name the options. This reduces accidental argument-order bugs in ML code.

### Scope

Python resolves names through local, enclosing, global, and built-in scopes. Closures capture values from enclosing scopes, which is useful for factories and decorators but can also surprise you when mutable state is captured.

### Recursion

Recursion is useful for tree-like problems but Python does not optimize arbitrary tail recursion, and deep recursion can hit recursion limits. Prefer iteration when it is clearer or safer.

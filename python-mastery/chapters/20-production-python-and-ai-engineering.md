# 20. Production Python and AI Engineering

> **Goal:** configuration, secrets, APIs, CLI tools, logging, packaging, Docker concepts, CI/CD, model services, monitoring, and architecture.
...
A practical AI application often has boundaries like:

`input → validation → business logic → model/service → persistence → observability`

Configuration belongs outside source code. Secrets belong in secret-management systems or environment variables, not in Git. Logging should capture useful context without leaking personal or sensitive information.
...
A small CLI is often a better first production interface for a reusable Python workflow than a notebook.

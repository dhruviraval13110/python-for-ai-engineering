...
> **Goal:** datasets, preprocessing, leakage, train/validation/test, sklearn pipelines, metrics, cross-validation, and reproducibility.
...
## The ML pipeline mental model

A reliable ML workflow separates:

1. data acquisition;
2. schema and quality validation;
3. train/validation/test splitting;
4. preprocessing fit only on training data;
5. model fitting;
...
Data leakage occurs when information unavailable at prediction time influences training or model selection. For example, computing a normalization statistic using the full dataset before splitting can allow test information to influence training. Scikit-learn pipelines help keep transformations tied to the training process.

### Metrics

Choose metrics based on the problem and costs of errors. Accuracy alone can be misleading for imbalanced classification. Precision, recall, F1, ROC-AUC, PR-AUC, MAE, RMSE, and calibration answer different questions.

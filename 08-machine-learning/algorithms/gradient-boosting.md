...
## What it solves
Gradient Boosting is useful when the goal is to **add weak learners that correct previous errors**.
...
**Core formulation:** `F_m(x)=F_{m-1}(x)+ηh_m(x)`

**Training objective / evaluation idea:** `chosen differentiable loss`
